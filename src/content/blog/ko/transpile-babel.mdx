...
title: '트랜스파일과 바벨'
hidden: true
description: 'Transpile 과 Babel 에 대해 설명합니다.'
pubDate: 'Sep 11 2021'
heroImage: '/content/blog/astro/hero.png'
---

## 여전한 Internet Explorer

웹 프론트엔드 개발자들의 귀찮은 점 중 하나는, 오래된 브라우저에서도 우리가 만든 앱이 잘 동작하게 하기 위해 노력해야 한다는 점 입니다. 사실 요즘은 브라우저들이 워낙 잘 만들어 진 데다, 대게 자동 업데이트 기능을 지원하기 때문에 구형 브라우저를 사용하는 사용자가 많지는 않습니다.

하지만 언제나 그렇듯 국내에서는 아직까지도 기업이나 관공서에서 `Internet Explorer`(이하 IE) 기반의 브라우저를 사용하고 있습니다. 저는 군생활을 행정병으로 지냈었는데, 2018년 전역 당시 육군에서 표준으로 사용하고 있는 브라우저는 IE8 버전 이었습니다. 요즘 세상에 어떻게 저 브라우저를 쓰느냐 하시겠지만, 군은 모든 업무 시스템이 IE8 의 `ActiveX` 기술을 기반으로 개발되어 있어서 이를 단번에 전환 하기에도 어려운 속사정이 있었습니다.

마이크로소프트는 최신 윈도우 11부터, IE 를 기본적으로 제거한다는 방침을 내놓았고 많은 소식통에서 드디어 IE 가 사라진다고 환영하고 있습니다. 하지만 사실 이 얘기는 윈도우 10의 출시 때에도 나왔던 이야기입니다.

윈도우10 에서 IE 가 보조프로그램으로 격하 되었고 새로운 `Edge` 브라우저가 내장되게 되면서 드디어 IE 가 역사속으로 사라진다며 기사가 나왔었습니다. 하지만 IE 는 아직까지 건재하며 잘 사용 되고 있습니다. 윈도우 11 역시도 `Edge` 브라우저 내에서 IE 호환성 모드를 제공할 예정이기 때문에 IE 는 계속해서 사용될 것 입니다.

저는 개인적으로 IE 가 기업과 관공서에서 까지 완전히 퇴출되는 시기를 빠르면 2040년 쯤으로 바라보고 있습니다. 이것도 확신이 서지 않는 이유는 한 조직의 업무 시스템이라는 것은 결코 쉽게 바뀌지 않기 때문입니다. 조직 입장에서는 브라우저를 바꾸게 되면 함께 사용중인 다른 모든 소프트웨어도 교체해야하기 때문에 비용 적으로 적지 않은 부담이 생기게 됩니다.

이러한 이해 관계의 결과로 개발자들은 기업이나 관공서에서 근무하거나, 그곳에 납품해야하는 제품을 만들어야 한다면 오래된 브라우저를 위해 여전히 구식 문법을 사용해 개발을 진행해야 합니다.
...
JavaScript 는 구형 문법과 신식 문법의 개발자 경험의 차이가 굉장히 큰 언어입니다. 최근들어 프론트엔드 개발을 시작한 분이시라면, 아마 `class` 와 `async & await` 같은 문법에 익숙할 것인데 이는 모두 IE11 이하의 브라우저에서는 사용할 수 없습니다.

결국 IE11 에서 동작시키기 위해서는 `class` 를 `function` 문법으로, `async & await` 은 `promise callback` 문법으로 개발해야 합니다. 이를 코드로써 나타내보면 아래와 같습니다.
...
> `Transpile` 은 좀 더 넓은 범위에서의 코드 변환을 지칭하는 단어이지만, 이와 같이 구형 브라우저에서의 동작을 위해 비슷한 코드로 변환하는 과정 역시도 `Transpile` 이라 부릅니다.
> 
